  ...
  <CommonPage>
    <div style="display: flex; justify-content: center; align-items: center; margin-bottom: 16px;">
      <NTooltip trigger="hover">
        <template #trigger>
          <div
            style="text-align: center; font-weight: bold; font-size: 25px; cursor: pointer;"
      ...
      <n-dropdown trigger="click" :options="dropdownOptions" @select="handleDropdownSelect">
        <NButton style="margin-left: 16px;" type="primary">
          切换表格
        </NButton>
      </n-dropdown>
    </div>
    ...
    <NForm
      v-if="showSearchForm" :model="excessStandardsForm" label-placement="left" label-width="auto"
      :style="{ marginBottom: '16px' }"
    >
      <NCard title="图像巡检超限标准">
        <NDataTable
          :columns="t1Columns" :data="t1Data" :bordered="true" :single-line="false" :scroll-x="1000"
          :max-height="500"
        />
          ...
          <tbody>
            <tr>
              <td class="content-cell" rowspan="6">
                <ul>
                  <li v-for="(item, index) in contents[0]" :key="index">
                    {{ item }}
                  </li>
                </ul>
              </td>
              ...
              <td class="content-cell" rowspan="6">
                <ul>
                  <li v-for="(item, index) in contents[2]" :key="index">
                    {{ item }}
                  </li>
                </ul>
              </td>
        ...
        <NCard title="双块式无砟轨道道床伤损形式及伤损等级判定标准">
          <NDataTable
            :columns="t5Columns" :data="t5Data" :bordered="true" :single-line="false"
            :scroll-x="1000" :max-height="500"
          />
        </NCard>

        <NCard title="轨枕埋入式无砟道床伤损形式及伤损等级判定标准">
          <NDataTable
            :columns="t5Columns" :data="t6Data" :bordered="true" :single-line="false"
            :scroll-x="1000" :max-height="500"
          />
        </NCard>

        <NCard title="弹性支承块式无砟道床伤损形式及伤损等级判定标准">
          <NDataTable
            :columns="t5Columns" :data="t7Data" :bordered="true" :single-line="false"
            :scroll-x="1000" :max-height="500"
          />
        </NCard>
      </NCard>
    </NForm>
    <NCard v-if="showSearchForm" :bordered="true" class="card-style">
      <NForm
        v-if="showSearchForm" :model="searchForm" label-placement="left" label-width="auto"
        :style="{ marginBottom: '16px' }"
      >
        <NGrid :cols="4" :x-gap="12" :y-gap="8">
          <NFormItemGi v-for="field in filterFields" :key="field.key" :label="field.label">
            <NInput v-model:value="searchForm[field.key]" :placeholder="`请输入${field.label}`" clearable />
          </NFormItemGi>
          <NFormItemGi>
            <NSpace>
              <NButton type="primary" style="margin-left: 50px;" @click="handleSearch">
                搜索
              </NButton>
    ...
    <NDataTable
      :columns="columns" :data="tableData" :bordered="true"
...
<script setup>
import { NButton, NCard, NDataTable, NForm, NFormItemGi, NGrid, NInput, NSpace, NTable, NTooltip } from 'naive-ui'
