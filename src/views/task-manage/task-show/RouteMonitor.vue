  ...
  <div class="route-monitor">
    <div ref="container" class="preview-container">
      <div class="timeline-wrapper">
        <!-- 渲染节点 -->
        <div
          v-for="(node, index) in nodes" :key="index" class="timeline-node"
        ...
        <!-- 渲染连接线 -->
        <div
          v-for="(connection, index) in connections" :key="`conn-${index}`" :ref="el => connectionLines[index] = el"
          class="connection-line"
        >
          <div class="progress-line" :style="getProgressStyle(connection)" />
        </div>
      </div>
    </div>
  </div>
...
// 获取连接线进度样式
function getProgressStyle(connection) {
  return {
...
// 根据里程更新进度
function updateProgressByMileage() {
