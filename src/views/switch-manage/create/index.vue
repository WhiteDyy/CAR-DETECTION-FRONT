          ...
          <NGrid :cols="24" :x-gap="16">
            ...
            <NGi :span="8">
              <NFormItem label="录入时间" path="recordedAt">
                <NDatePicker v-model:value="formData.recordedAt" type="datetime" clearable style="width: 100%" />
              </NFormItem>
            </NGi>
            <NGi :span="8">
              <NFormItem label="采样频率" path="samplingFrequency">
                <NInputNumber v-model:value="formData.samplingFrequency" :min="1" :precision="0" style="width: 100%" />
              </NFormItem>
            </NGi>
            <NGi :span="16">
              <NFormItem label="备注" path="remark">
                <NInput v-model:value="formData.remark" type="textarea" :rows="2" placeholder="可填写线路、施工状态、人工测量说明等" />
              </NFormItem>
            </NGi>
          </NGrid>
...
const rules = {
  ...
  samplingFrequency: { required: true, type: 'number', message: '请输入采样频率', trigger: 'change' },
}

function calculateStartPulse(startMileage) {
  const mileage = Number(startMileage)
  const samplingFrequency = Number(formData.samplingFrequency)
  if (!Number.isFinite(mileage) || mileage < 0 || !Number.isFinite(samplingFrequency) || samplingFrequency <= 0)
    return null
  const pulse = mileage * samplingFrequency / (200 * Math.PI)
...
const nodeColumns = [
  ...
  { title: '节点名称', key: 'stepName', width: 180 },
  {
    ...
    render: row => h(NInputNumber, {
      ...
      onUpdateValue: (v) => {
        row.startMileage = v
        row.startPulse = calculateStartPulse(v)
      },
    }),
  },
  {
...
const nodeColumnsReadonly = [
  { title: '工步', key: 'stepId', width: 70 },
  { title: '节点名称', key: 'stepName', width: 180 },
  { title: '起始点(m)', key: 'startMileage', width: 130 },
...
function resetForm() {
  ...
  formData.samplingFrequency = 300
  formData.remark = ''
...
function buildPayload() {
  ...
  return {
    ...
    samplingFrequency: formData.samplingFrequency,
...
async function handleSubmit() {
  ...
  if (!payload.nodes.length) {
    window.$message?.warning('请至少录入一个节点的脉冲或里程范围')
    return
  }

  submitting.value = true
  try {
    await request.post('/switch_manual_batches', payload)
...
async function fetchBatchList() {
  ...
  try {
    ...
    const res = await request.get('/switch_manual_batches', { params })
    ...
    if (Array.isArray(data)) {
      ...
      batchList.value = data?.pageData || data?.list || data?.rows || []
    ...
    window.$message?.error(e?.message || '查询失败')
  }
  finally {
    loading.value = false
  }
